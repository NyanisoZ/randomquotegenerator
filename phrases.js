    ...
    var phrases = [
        {
            hour : 0,
            content : [
                "You should be a monster, an absolute monster and then you should learn how to control it. It's better to be a warrior in a garden than a gardner in a war",
        ...
        ,
        {
            hour: 13,
            content : [
                "Pick up your famn suffering and then bear it, and try to be a good peson so you don't make it worse",
...
this.pickCurrent = function() {
    returnthis.pikOne(new Date().getHours());
}

this.pickOne = function(hourOfDay) {
    for(var x in phrases) {
        if(phrases[x].hour === hourOfDay) {
            return phrases[x].content[Math.floor((Math.random() * phrases[x].content.length))]
        }
    }
}
